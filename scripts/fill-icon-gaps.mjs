/**
 * Complete an icon collection's matrix: every icon at 16/20/24/28/32/48, in
 * Standard, Outline and Filled.
 *
 ...
 *
 * Sizes a collection already holds beyond the six are kept, not trimmed: the
 * 64 on app tiles and the 12 on a dozen product icons are authored artwork,
 * and the six are a floor rather than a ceiling.
 *
 * Usage: node scripts/fill-icon-gaps.mjs --collection=app [--dry] [--refresh]
 */
...
const COLLECTION = (process.argv.find((a) => a.startsWith('--collection=')) || '--collection=product')
  .split('=')[1];
const REFRESH = process.argv.includes('--refresh');

const manifest = JSON.parse(readFileSync(join(ROOT, 'manifest.json'), 'utf8'));
const icons = manifest.assets.filter((a) => a.collection === COLLECTION && a.type === 'icon');
if (!icons.length) {
  console.error(`No icons in collection "${COLLECTION}".`);
  process.exit(1);
}
...
for (const asset of icons) {
  const dir = join(ROOT, 'assets/icons', COLLECTION, asset.id.split('.').slice(1).join('.'));
  ...
  for (const style of STYLES) {
    for (const size of SIZES) {
      ...
      if (haveInStyle.length) {
        ...
        // Standard is the preferred source, but four third-party brands ship
        // only a monochrome mark. Deriving from whatever they do have beats
        // leaving the row empty, and the cell is flagged either way.
        const sourceStyle = authored.standard.length
          ? 'standard'
          : STYLES.find((k) => authored[k].length);
        if (!sourceStyle) { failed.push(`${asset.id} ${style}:${size}: no artwork to derive from`); continue; }
        const std = authored[sourceStyle];
        const from = nearest(std, size) ?? std[0];
        const src = readFileSync(join(ROOT, meta.variants[sourceStyle][from]), 'utf8');
      ...
      const rel = `assets/icons/${COLLECTION}/${asset.id.split('.').slice(1).join('.')}/${style}-${size}.svg`;
  ...
  // The six are a floor. A size this asset already had — 64 on an app tile, 12
  // on a handful of product icons — is authored artwork and stays.
  const kept = new Set(SIZES);
  for (const style of STYLES) {
    for (const size of Object.keys(meta.variants[style] || {})) kept.add(Number(size));
  }
  const allSizes = [...kept].sort((a, b) => a - b);
  meta.sizes = allSizes;
  for (const style of STYLES) {
    if (!meta.variants[style]) continue;
    const sorted = {};
    for (const size of allSizes) if (meta.variants[style][size]) sorted[size] = meta.variants[style][size];
