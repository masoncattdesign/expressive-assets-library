 ...
 *
 * MIT licensed, Copyright (c) 2020 Microsoft Corporation. The licence requires
 * the copyright notice to travel with the artwork, so this writes THIRD-PARTY-
 * NOTICES.md and stamps every imported asset with a `source` block. Attribution
 * that lives only in a README nobody opens is attribution in name only.
 *
...
async function main() {
  ...
  /* The licence requires the notice to travel with the artwork. */
  const upstreamLicense = await readFile(join(FROM, 'LICENSE'), 'utf8').catch(() => null);
  await writeFile(
    join(ROOT, 'THIRD-PARTY-NOTICES.md'),
    `# Third-party notices\n\n` +
      `## Fluent System Icons\n\n` +
      `${written} assets in \`assets/icons/${COLLECTION}/\` are derived from\n` +
